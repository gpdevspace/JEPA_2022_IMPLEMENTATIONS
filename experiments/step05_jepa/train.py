...
from experiments.step05_jepa.model import (
    JEPA,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Step 5 JEPA with pluggable loss")
    ...
    for epoch in range(args.epochs):
        ...
        for x, y, labels in tqdm(loader, desc=f"Epoch {epoch}/{args.epochs-1}"):
            ...
            # Forward pass
            s_x, s_y, s_y_pred = model(x, y)

            # Compute loss
            loss, metrics = loss_fn(s_y, s_y_pred)
            ...
            for key in ["invariance", "variance_loss", "covariance_loss"]:
